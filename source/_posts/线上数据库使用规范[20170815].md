...
```
1. 库名以【d】开头(暂未遵循)
2. 表名以【tbl】开头,如：tblUserMsg
3. 字段名以【驼峰】命名风格命名
4. 索引  
  4.1 原则上必须定义PRIMARY KEY (`id`)  
  4.2 普通索引以【idx_】开头后面紧跟相关列，如：idx_col1_col2(KEY `idx_msgType_status` (`msgType`,`status`))  
  4.3 唯一索引以【uk_】开头， 如：uk_col1_col2(UNIQUE KEY `uk_adminUid_userUid` (`adminUid`,`userUid`))
  
## 1.2 长度限制
库名、表名、字段名禁止超过32个字符，需见名知意

## 1.3 临时库、表名须以tmp加日期为后缀
...
使用utf8
## 建表规范
所有表都必须要显式指定主键，每个字段都要有注释，以说明期作用。
例如：
