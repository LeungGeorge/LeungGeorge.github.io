...
gcache 是基于 Golang 实现的一个内存级 Cache 基础库。支持带失效时间的 Cache，目前包括 LFU（Least Frequency Used），LRU（Least Recently Used），ARC（Adaptive Replacement Cache）三种缓存策略。在这篇文章里，我们将主要解释键值对的读写操作，其他不常用操作（如：Len、Keys等等）可参考代码。
...
gcache 中 baseCache 是 gcache 的基础数据结构，包含缓存大小、回调、失效时间、读写锁、命中率等，由 cache.go/baseCache 结构表示：
...
用来构造缓存对象，以及各种个性化化配置、策略。由 cache.go/CacheBuilder 结构表示：  
...
列举 `loaderExpireFunc` （key 过期时的回调函数）的具体实现；其他回调（淘汰 key 时的回调、序列化回调、反序列化回调等等）的设置方式相同。
...
列举一个 `LRU 淘汰策略` 的设置（LFU，ARC 的设置方式相同）。
...
SimpleCache 中 simpleItem 由 lru.go/simpleItem 结构表示：
```go
...
SimpleCache 结构比较简单，通过 map 直接实现：
...
simpleItem 超时判断，其他（lruItem，lfuItem，arcItem）超时判断逻辑相同，即检查过期时间 expiration 是否在当前时间 now 之前。
...
1. 读取成功
执行 GET 操作，读取 key 为 k2 的键值对。缓存命中次数（hitCount）+1，k2 在被移动到淘汰链表首部。读取后成功后 LRUCache 状态（流程）：
![20191009201726.png](https://raw.githubusercontent.com/LeungGeorge/assets/master/images/20191009201726.png)
![20191009201812.png](https://raw.githubusercontent.com/LeungGeorge/assets/master/images/20191009201812.png)
![20191009201856.png](https://raw.githubusercontent.com/LeungGeorge/assets/master/images/20191009201856.png)

2. 读取失败（访问不存在的键）
执行 GET 操作，读取 key 为 k4 的键值对。缺失次数（missCount）+1，淘汰链表顺序不变，LRUCache 状态：
