...
工作原理图：  
![image](source/assets/images/基于人口统计学的推荐原理%202.jpg)   
系统对每个根据每个用户的基本信息，计算用户的相似度。比如系统通过计算发现用户A和C比较相似。就会把A喜欢的物品推荐给C。

## 基于内容的推荐
工作原理图：  
![image](source/assets/images/基于内容的推荐原理%202.jpg)  
...
**相似度计算：**   
（1）Jaccard公式  
![image](source/assets/images/jaccard公式%202.png)  
（2）余弦相似度  
![image](source/assets/images/余弦相似度%202.png)   
其中，N(u),N(v)分别代表用户u，v的兴趣集合。  
**用户行为记录表：**  
![image](source/assets/images/用户行为记录表%202.png)   
如图，用户A对物品{a, b, d}有过行为，用户B对物品{a, c}有过行为，利用余弦相似度公式计算用户A和用户B的兴趣相似度为:  
![image](source/assets/images/用户A和用户B的相似度%202.png) 
**用户相似度矩阵：**
...
**改进型用户相似度计算**  
![image](source/assets/images/改进型用户相似度计算%202.png)  
该方法通过 1/log<sub>2(</sub>1+|N(i)|) 惩罚了用户u，v共同兴趣列表中热门物品对他们相似度的影响。  
**用户对物品感兴趣程度公式：**  
![image](source/assets/images/用户对物品的感兴趣程度%202.png)  
...
**相似度计算：**  
![image](source/assets/images/基于内容的相似度计算公式%202.png)  
**相似度矩阵：**  
![image](source/assets/images/基于内容的相似度矩阵%202.png)   
...
**豆瓣图书推荐：**  
![image](source/assets/images/豆瓣猜%202.png)    
**豆瓣电影推荐：**  
![image](source/assets/images/豆瓣电影推荐%202.png)   
...
[comment]:**感兴趣的人：**  
[comment]:![image](source/assets/images/感兴趣的人推荐%202.png)   
[comment]:**相关问题：**  
[comment]:![image](source/assets/images/站内相关问题推荐%202.png) 
