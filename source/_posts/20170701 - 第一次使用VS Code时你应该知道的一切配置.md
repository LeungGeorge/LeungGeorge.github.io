...
![](assets/images/f0fd78115a0e4f63d79c3b4ca81aa68c_MD5.png)
...
- 支持第三方插件，功能强大，生态系统完善。
- 自带丰富的调试功能。
- 自带 emmet：支持代码自动补全，快速地生成简单的语法结构。要知道，这个功能在 Sublime Text 中，得先安装插件才行。
...
![](assets/images/797d95a7e6d484e1a677949c7ba77611_MD5.png)

上图中，直接点击 download，一键下载安装即可。

VS Code 支持以下平台：

![](assets/images/03c92ea8f3b30d505f50f80e58b32e02_MD5.png)
...
| -------------------- | ------------------------- | --------------------------------------------- | -------------------- |
| **Cmd + Shift + P**  | **Ctrl + Shift + P**，F 1 | 显示命令面板                                  |                      |
...
| **Cmd + G**         | **F 3**             | 在当前文件中搜索代码，光标仍停留在编辑器里 | 很巧妙 |
...
![](assets/images/ddee30cdaff5ff8e312540357d323974_MD5.png)

### 10、快捷键列表

你可以点击 VS Code 左下角的齿轮按钮，效果如下：

![](assets/images/37ee71765693f919b70dac202541cff7_MD5.png)

上图中，在展开的菜单中选择「键盘快捷方式」，就可以查看和修改所有的快捷键列表了：

![](assets/images/f042d480abd001b4ec868c9a999d23a3_MD5.png)
...
![](assets/images/1fa1dcb852d5c860d410b09053d82fdf_MD5.gif)
...
![](assets/images/296c02d8d4353748b74197720f9d9fbe_MD5.png)
...
![](assets/images/2d06975b1384253fa1dba9722da5c375_MD5.png)
...
![](assets/images/76b2a01cc2d8d543cca08070a76f9332_MD5.png)

### 5、使用命令行启动 VS Code

（1）输入快捷键「Cmd + Shift + P 」，选择 `install code command`：

![](assets/images/47da18bb955d3e4c466fd87f6593adf1_MD5.png)
...
在修改 VS Code 配置之前，我们需要知道，在哪里可以找到配置项的入口。

**方式 1**：Mac 用户选择菜单栏「Code--> 首选项-->设置」，即可打开配置项：

![](assets/images/3b03083f45008c45c6043dff00cfdbee_MD5.png)

**方式 2**：点击软件右下角的设置图标：

![](assets/images/ebf8bc9b1c391d280eaa4259c7fac5f6_MD5.png)

### 1、修改颜色主题

选择菜单栏「Code --> 首选项 --> 颜色主题」：

![](assets/images/f7420ff9d06cb377401af6e533a66fe6_MD5.png)

在弹出的对话框中，挑选你一个你喜欢的的颜色主题吧：

![](assets/images/8f2e466fb5b774ad2eb8f183cb4a166a_MD5.png)

### 2、面包屑（Breadcrumb）

打开 VS Code 的设置项，选择「用户设置 -> 工作台 -> 导航路径」，如下图所示：

![](assets/images/a2eee32c375f13af68503aaf1a823656_MD5.png)

上图中，将红框部分打钩即可。

设置成功后，我们就可以查看到当前文件的「层级结构」，非常方便。如下图所示：

![](assets/images/a2c72734cc4c75ea81814ab71c3c36f0_MD5.png)
...
![](assets/images/8d4c306f37176a65ed0e2ad40f623372_MD5.png)
...
![](assets/images/9910317bdfc267d858537181daae2c0e_MD5.png)
...
![](assets/images/53d142c3a713aa1ba0740e3580387bf2_MD5.png)
...
![](assets/images/82f038e9ed2f67cab8bda00619391162_MD5.png)
...
![](assets/images/c415b637c12b130d2b7c53333d3fefc9_MD5.png)

- **editor. InsertSpaces**：按 Tab 键时插入空格（默认）。截图如下：

![](assets/images/73408a2f41f0f58f1ca32e160a3d7f66_MD5.png)

- **editor. TabSize**：一个制表符默认等于四个空格。截图如下：

![](assets/images/a7de6f353e7f300e237e81dd3fd12ddf_MD5.png)
...
![](assets/images/2db4234fa90f2550f9d7581579139466_MD5.png)
...
![](assets/images/665c853105a9fbc224a606715c84c209_MD5.png)
...
![](assets/images/9bdc8700ae25d66099a69f093b60a47d_MD5.png)

VS Code 自带的对比功能并不够强大，我们可以安装插件 `compareit`，进行更丰富的对比。比如说，安装完插件 `compareit` 之后，我们可以将「当前文件」与「剪切板」里的内容进行对比：

![](assets/images/ab8f6322e9401d1fbe0ede49804dc611_MD5.png)

### 13、查找某个函数在哪些地方被调用了

比如我已经在 `a.js` 文件里调用了 `foo()` 函数。那么，如果我想知道 `foo()` 函数在其他文件中是否也被调用了，该怎么做呢？

做法如下：在 `a.js` 文件里，选中 `foo()` 函数（或者将光标放置在 `foo()` 函数上），然后按住快捷键「Shift + F 12」，就能看到 `foo()` 函数在哪些地方被调用了，比较实用。
...
当我们尝试去修改某个函数（或者变量名）时，我们可以把光标放在上面，然后按下「F 2」键，那么，这个函数（或者变量名）出现的地方都会被修改。
...
- Cmd + G（Win 用户是 F 3）：在当前文件中搜索，光标仍停留在编辑器里

另外，你可能会注意到，搜索框里有很多按钮，每个按钮都对应着不同的功能，如下图所示：

![](assets/images/eda24e1fe7a4ff2544a2bd4d5999ee32_MD5.png)
...
![](assets/images/1c50ec34411d31f57e3c30d253b8cdd0_MD5.png)
...
![](assets/images/b3d5ce5e564f91158010cd652f849211_MD5.png)
...
![](assets/images/b91ec66cafdc6e81a1dd40cd8145ef0f_MD5.png)
...
![](assets/images/9846a9c2a458ad0cbb9cf55e5ecdeb78_MD5.png)

那么，就可以创建多层子文件夹，效果如下：

![](assets/images/23e1d773c91b58aa72be907357f4c97f_MD5.png)
...
我们可以按下「Ctrl + \`」打开 VS Code 自带的终端。我认为内置终端并没有那么好用，我更建议你使用第三方的终端 **item 2**。
...
![](assets/images/096895349c9a0d88860515af67f274e0_MD5.png)
...
![](assets/images/556a3bbe3bc7d699b63ad589b70f3d69_MD5.png)
...
（3）在 VS Code 的当前文件里，选择「右键 -> upload」，就可以将本地的代码上传到指定的 ftp 服务器上（也就是在上方 `host` 中配置的服务器 ip）。

我们还可以选择「右键 -> Diff with Remote」，就可以将本地的代码和 ftp 服务器上的代码做对比。

### 28、设置 tab 的缩进

在配置里搜索 `Detect Indentation`，修改为 false。参考链接：[https://www.yisu.com/zixun/327399.html](https://www.yisu.com/zixun/327399.html)

## 六、三头六臂：VS Code 插件推荐

VS Code 有一个很强大的功能就是支持插件扩展，让你的编辑器仿佛拥有了三头六臂。

![](assets/images/9c4f6adb9d8814c4314572fd1eaf19bc_MD5.png)
...
![](assets/images/be7d4dbdb4bd406e053d1435d6f73fb1_MD5.png)

安装了 GitLens 之后是这样的：

![](assets/images/dfaadee6df434ef4d2f7262aae25efcd_MD5.png)
...
![](assets/images/5e49cdb62c66c778d483e04421658b19_MD5.png)
...
![](assets/images/cfc5d92b961e714bd3baf19864988154_MD5.png)
...
### 19、ES 7 React/Redux/GraphQL/React-Native snippets
...
### 24、JavaScript (ES 6) code snippets
...
![](assets/images/a66ddadc748bd3ec22455b886dd4e14a_MD5.jpg)
...
![](assets/images/4596a15fc12009c859890562a7a6da9b_MD5.png)
...
![](assets/images/3ddcfb4b43003f9e880646cb378586be_MD5.png)
...
![](assets/images/f9267d03a09b236f3c2f4c529b44241e_MD5.png)
...
![](assets/images/97d659c506732673cd405101d82a5e34_MD5.png)
...
![](assets/images/9afc29e1d1942efceb39446f8746ab75_MD5.png)

上图中，点击红框部分，登录后就可以分享你的工作空间了。

![](assets/images/402015daf55df4cb8be60268a725dc1d_MD5.png)
...
![](assets/images/f4382ac255d90d7227293e909fa03514_MD5.png)

（2）选择需要同步的配置：

![](assets/images/7fa9651f81faddcf4c1b6931e9b9424f_MD5.png)

（3）通过 Microsoft 或者 GitHub 账号登录。上图中，点击“登录并打开”，然后弹出如下界面：

![](assets/images/fadc062830fb52bb2c86fd8bf0dd75cd_MD5.png)

上图中，使用微软账号或者 GitHub 账号登录：

![](assets/images/7b08f865987a038848cb4b9c052dd5bb_MD5.png)

（4）同步完成后，菜单栏会显示“首先项同步已打开”，最左侧也会多出一个同步图标，如下图所示：

![](assets/images/aa67710230396c579003e1dd9b8c26ae_MD5.png)

2、**管理同步**：

（1）点击菜单栏「Code --> 首选项 --> 设置同步已打开」，会弹出如下界面，进行相应的同步管理即可：

![](assets/images/7c073dce63bf3dfbfa40e71ec01035f2_MD5.png)
...
![](assets/images/92dae29fa549f8b782324658b2cd6ab3_MD5.png)

上图中，我们点击最新的那条记录，就可将云端的最新配置同步到本地：

![](assets/images/3e2c69fa1d56059de248241253fa1c5d_MD5.png)
...
![](assets/images/3c3113763bfcc9759ba5ea08b48fe997_MD5.png)
